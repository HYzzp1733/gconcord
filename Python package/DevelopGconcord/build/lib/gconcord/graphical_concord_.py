...
class GraphicalConcord():
    ...
    def __init__(self, lam1 = 0.1, lam2 = 0, method = 'ista',
                tol = 1e-5, maxit = 100, steptype = 0,
                assume_centered = False, assume_scaled = False):
        self.lam1 = lam1
        self.lam2 = lam2
        self.method = method
        self.tol = tol
...
class GraphicalConcordCV(GraphicalConcord):
    ...
    def __init__(self, lam1s = None, lam2s = 0, 
                 random_state = None, shuffle = False, folds = 3,
                 tol = 1e-5, maxit = 100, steptype = 0, method = 'ista', 
                 assume_centered = False, assume_scaled = False):
        super().__init__(
    ...
    def _predrisk(self, omega, data):
        ...
        return np.linalg.norm(arg, ord = 'fro') ** 2 / data.shape[0]
    
    def _candidate(self):
        """Determine candidate values for lam1 and lam2
        """
        if self.lam1s is None:
            self.lam1s = 10 ** np.linspace(-1.2, 0, 10) - 10 ** (-1.2)
        if self.lam2s is None:
            self.lam2s = 10 ** np.linspace(-1.2, 0, 4) - 10 ** (-1.2)
